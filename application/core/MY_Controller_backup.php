...
function sms_jazz($to,$message){
	...
	if($response=='Message Sent Successfully!'){
		$success = true;
	$query="INSERT INTO `sms`( `mobile_number`, `message`,  `sms_status`, `comment`, `date`) 
				value ( '".$to."', '".$message."', '1', '".$response."', '".date("Y-m-d G:i:s", time())."')";
		$this->db->query($query);
	}else{
		$success = false;
		$query="INSERT INTO `sms`( `mobile_number`, `message`,  `sms_status`, `comment`, `date`) 
				value ( '".$to."', '".$message."', '0', '".$response."', '".date("Y-m-d G:i:s", time())."')";
		$this->db->query($query);
		}
	...
	if(substr($result->QuickSMSResult,0,22)=='Submitted Successfully'){
		$success = true;
		$query="INSERT INTO `sms`( `mobile_number`, `message`,  `sms_status`, `comment`, `date`) 
				value ( '".$to."', ".$this->db->escape($message).", '1', ".$this->db->escape($result->QuickSMSResult).", '".date("Y-m-d G:i:s", time())."')";
		$this->db->query($query);
		}else{
			$success = false;
			$query="INSERT INTO `sms`( `mobile_number`, `message`,  `sms_status`, `comment`, `date`) 
				value ( '".$to."', ".$this->db->escape($message).", '0', ".$this->db->escape($result->QuickSMSResul).", '".date("Y-m-d G:i:s", time())."')";
		$this->db->query($query);
			}
